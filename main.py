...
============
    ...
    -ep, --encrypt-print    Encrypt the file and output.
    -dp, --decrypt-print    Decrypt the file and output.
    ...
    file        -e file.txt     Encrypt the file (test.txt) with a password.
    file        -d file.txt     Decrypt the file (test.txt) with a password.
    file        -ep file.txt    Encrypt the contents of the file (test.txt) with a password and output.
    file        -dp file.txt    Decrypt the contents of the file (test.txt) with a password and output.'''
...
===============
    ...
    history     -c                  The history file will be cleared. (.mf_history).
...
def banner():
    clear()
    print(__banner__)
    message(f'Metros Framework version {__version__}')
    message('Github: https://github.com/john-metrossoftware/metrosframework')
    warning("To access help, use the 'help' command.", start='\n', end='\n\n')
...
class Main:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.file_history = os.path.abspath(f'{self.current_path}/.mf_history')
        self.terminal_session = prompt_toolkit.PromptSession(
            history=prompt_toolkit.history.FileHistory(self.file_history))
        self.terminal_text = prompt_toolkit.HTML(f'<u><b>mf1</b></u> > ')
    ...
    def input_validation(self):
        ...
        elif self.input_line_split[0].lower() == 'banner':
            print(__banner__)
        else:
        ...
        try:
            if self.input_line_split[1].lower() == '--create' or self.input_line_split[1].lower() == '-c':
                ...
                try:
                    ...
                    error('You did not enter a file name.')
            elif self.input_line_split[1].lower() == '--encrypt' or self.input_line_split[1].lower() == '-e':
                try:
                    if not os.path.isfile(self.input_line_split[2]):
                        error(f'Could not find file: {self.input_line_split[2]}')
                    elif os.path.isfile(self.input_line_split[2]):
                        while True:
                            encrypt_password_0 = prompt_toolkit.prompt(
                                prompt_toolkit.HTML('Enter the password to <u>encrypt</u> the file: '),
                                is_password=True)
                            encrypt_password_1 = prompt_toolkit.prompt('Confirm your password: ', is_password=True)
                            if encrypt_password_0 == encrypt_password_1:
                                encrypt = cryptocode.encrypt(open(self.input_line_split[2],
                                                                  encoding='utf-8').read(),
                                                             encrypt_password_0)
                                if not encrypt:
                                    error(f'Unable to encrypt file: {self.input_line_split[2]}')
                                else:
                                    try:
                                        write_file = open(self.input_line_split[2], 'w+')
                                        write_file.write(encrypt)
                                        write_file.close()
                                    except Exception as encrypt_error:
                                        error(f'Failed to write the result to a file: {self.input_line_split[2]}\n')
                                        error(encrypt_error)
                                    else:
                                        message(f'(cryptocode) The file is encrypted: {self.input_line_split[2]}')
                                        break
                            else:
                    ...
                    return
            elif self.input_line_split[1].lower() == '--decrypt' or self.input_line_split[1].lower() == '-d':
                try:
                    if not os.path.isfile(self.input_line_split[2]):
                        error(f'Could not find file: {self.input_line_split[2]}')
                    elif os.path.isfile(self.input_line_split[2]):
                        while True:
                            decrypt_password_0 = prompt_toolkit.prompt(
                                prompt_toolkit.HTML('Enter the password to <u>decrypt</u> the file: '),
                                is_password=True)
                            decrypt = cryptocode.decrypt(open(self.input_line_split[2],
                                                              encoding='utf-8').read(),
                                                         decrypt_password_0)
                            if not decrypt:
                                warning('Passwords do not match...')
                                ...
                                try:
                                    write_file = open(self.input_line_split[2], 'w+')
                                    write_file.write(decrypt)
                                    write_file.close()
                                except Exception as decrypt_error:
                                    error(f'Failed to write the result to a file: {self.input_line_split[2]}\n')
                                    error(decrypt_error)
                                else:
                                    message(f'(cryptocode) The file is decrypted: {self.input_line_split[2]}')
                                    break
                except IndexError:
                    print('You did not enter a file name.')
                except KeyboardInterrupt:
                    return
            elif self.input_line_split[1].lower() == '--encrypt-print' or self.input_line_split[1].lower() == '-ep':
                try:
                    if not os.path.isfile(self.input_line_split[2]):
                        error(f'Could not find file: {self.input_line_split[2]}')
                    elif os.path.isfile(self.input_line_split[2]):
                        while True:
                            encrypt_password_0 = prompt_toolkit.prompt(
                                prompt_toolkit.HTML('Enter the password to <u>encrypt print</u> the file: '),
                                is_password=True)
                            encrypt_print = cryptocode.encrypt(open(self.input_line_split[2],
                                                                    encoding='utf-8').read(),
                                                               encrypt_password_0)
                            if not encrypt_print:
                                error(f'Unable to print encrypt file: {self.input_line_split[2]}')
                            else:
                                print(encrypt_print)
                                break
                except IndexError:
                    ...
                    return
            elif self.input_line_split[1].lower() == '--decrypt-print' or self.input_line_split[1].lower() == '-dp':
                try:
                    if not os.path.isfile(self.input_line_split[2]):
                        error(f'Could not find file: {self.input_line_split[2]}')
                    elif os.path.isfile(self.input_line_split[2]):
                        while True:
                            decrypt_password_0 = prompt_toolkit.prompt(
                                prompt_toolkit.HTML('Enter the password to <u>decrypt print</u> the file: '),
                                is_password=True)
                            decrypt_print = cryptocode.decrypt(open(self.input_line_split[2],
                                                                    encoding='utf-8').read(),
                                                               decrypt_password_0)
                            if not decrypt_print:
                                error(f'Unable to print decrypt file: {self.input_line_split[2]}')
                            else:
                                print(decrypt_print)
                                break
                except IndexError:
    ...
    def help(self):
        try:
            ...
            elif self.input_line_split[1].lower() == 'banner':
                print(__help_banner__)
    ...
    def history(self):
        ...
        try:
            if self.input_line_split[1].lower() == '--clear' or self.input_line_split[1].lower() == '-c':
                self.history_remove_history_file()
    ...
    def history_remove_history_file(self):
